...
/** 自动渲染导航栏 */
export const autoRenderNavbar = ({ exclude = [], excludeFile = [], source = 'src' } = defaultReadConfig) => {
	console.log('解析路径: ', path.resolve(process.cwd(), source))
	/** 读取所有子文件 */
	...
	for (let h1 of navbars) {
		...
		for (const h2 of level2) {
			...
			if (level3.includes('index.md')) {
				...
				const MarkdownFiles = level3.filter((file) => path.extname(file) === '.md')
				h1.items!.push({
					text: h2,
...
/** 自动渲染左边侧边栏 */
export const autoRenderSidebar = ({ exclude = [], excludeFile = [], source = 'src' } = defaultReadConfig) => {
	/** 侧边栏对象 */
	...
	for (let h1 of level1) {
		...
		for (let h2 of level2) {
			...
			// sidebars[key] = [];
			// Object.assign(sidebars, { [key]: [] })
			const secondarybar: DefaultTheme.SidebarItem = {
				text: h2,
				collapsed: false,
				items: [],
			}
			...
			for (let h3 of level3) {
				...
				/** 判断是否为markdown文件, 是markdown文件再添加 */
				if (path.extname(h3) === '.md') {
					const filename = path.basename(h3, '.md')
					secondarybar.items?.push({
						text: filename,
						link: `/${h1}/${h2}/${h3}`,
					})
				}
			}
			;(sidebars[key] as DefaultTheme.SidebarItem[]).push(secondarybar)
		}
	}
	return sidebars
}
